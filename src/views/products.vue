...
const filtered = computed(() => {
  const base = products.value.filter(p => p.type === 1)
...
function displayTitle(p: Product){ return '[供] ' + (p.title || '') }
...
const formErrors = computed(() => {
  ...
  if (!Number.isFinite(priceNum) || priceNum <= 0) errs.price = '单价需为大于0的数字'
  if (!Number.isFinite(stockNum) || stockNum <= 0 || !Number.isInteger(stockNum)) errs.stockQuantity = '库存需为大于0的整数'
  if (!content) errs.content = '介绍内容为必填项'
  if (!picture) errs.picture = '请上传商品图片'
...
// 状态恢复与保存逻辑
onMounted(async () => {
  const stateStr = sessionStorage.getItem('PRODUCTS_STATE')
  if (stateStr) {
    try {
...
onBeforeRouteLeave((to, from, next) => {
  if (to.name === 'product-detail') {
    ...
    sessionStorage.setItem('PRODUCTS_STATE', JSON.stringify(state))
  } else {
...
<template>
  <div class="page">
    <SiteHeader :extra-nav-link="{ label: '我的货源管理', to: { name: 'products', query: { manage: '1' } } }" />
    <section class="search-bar">
      <input v-model="keyword" class="search-input" placeholder="搜索货源…" />
    </section>
    <section class="section-title">{{ isManage ? '我的货源列表' : '货源列表' }}</section>
    <div v-if="isManage" class="manage-toolbar">
      <button class="btn" @click="openCreate">发布货源</button>
    </div>
    ...
    <div v-else class="grid">
      <article v-for="p in filtered" :key="p.productId" class="card" :class="{ 'no-pointer': isManage }" @click="!isManage && goDetail(p.productId)">
        <div class="thumb">
          ...
          <span class="badge">货源</span>
        </div>
        <div class="meta">
          <h3 class="title" :title="displayTitle(p)">{{ displayTitle(p) }}</h3>
          <p class="desc">{{ p.content }}</p>
          <div class="bottom"><span class="price">¥{{ Number(p.price).toFixed(2) }}</span></div>
      ...
      <div v-if="!filtered.length" class="empty">暂无货源</div>
    </div>

    <div v-if="showForm" class="modal-backdrop" @click.self="showForm=false">
      <div class="modal">
        <h3>{{ formMode === 'create' ? '发布货源' : '编辑货源' }}</h3>
        <div class="form">
          <label>标题
            <input v-model="form.title" placeholder="如：优质小麦" />
